...
function parseSillyTavernBook(raw: Record<string, unknown>): CharacterBook | null {
  ...
  if (Array.isArray(rawEntries)) {
    ...
    // SillyTavern standalone lorebooks commonly store entries as an object
    // keyed by numeric strings rather than as a JSON array.
...
/** Build importable items from a standalone lorebook without card context. */
...
/**
 * Parse a SillyTavern standalone world-info / lorebook JSON file.
 * Character cards and Errata exports are explicitly excluded.
 */
export function parseSillyTavernLorebook(text: string): ParsedLorebook | null {
  ...
  // A card can also contain entries; card parsing owns that format.
  if (parseCardJson(text) !== null) return null

  const book = parseSillyTavernBook(raw)
  if (!book || book.entries.length === 0) return null

  return { book, items: buildLorebookItems(book) }
}

/** Quick detection for standalone SillyTavern lorebook JSON. */
