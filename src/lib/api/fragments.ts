...
export const fragments = {
  ...
  update: (
    storyId: string,
    fragmentId: string,
    data: { type?: string; name: string; description: string; content: string; sticky?: boolean; order?: number; placement?: 'system' | 'user'; meta?: Record<string, unknown>; reason?: string },
    branchId?: string,
  ) =>
    apiFetch<Fragment & { idChanged?: boolean }>(
      `/stories/${storyId}/fragments/${fragmentId}${branchId ? `?branch=${encodeURIComponent(branchId)}` : ''}`,
