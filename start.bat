...
"%POWERSHELL_EXE%" -NoLogo -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ErrorActionPreference='Stop'; [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $root=Split-Path -Parent $env:ERRATA_NODE_HOME; New-Item -ItemType Directory -Force -Path $root | Out-Null; Invoke-WebRequest -UseBasicParsing -Uri $env:ERRATA_NODE_URL -OutFile $env:ERRATA_NODE_ZIP; $actual=(Get-FileHash -LiteralPath $env:ERRATA_NODE_ZIP -Algorithm SHA256).Hash.ToLowerInvariant(); $expected=($env:ERRATA_NODE_SHA256).ToLowerInvariant(); if($actual -ne $expected){ throw ('Node.js archive SHA256 mismatch. Expected '+$expected+' but got '+$actual) }; if(Test-Path -LiteralPath $env:ERRATA_NODE_HOME){ Remove-Item -LiteralPath $env:ERRATA_NODE_HOME -Recurse -Force }; Expand-Archive -LiteralPath $env:ERRATA_NODE_ZIP -DestinationPath $root -Force; Remove-Item -LiteralPath $env:ERRATA_NODE_ZIP -Force -ErrorAction SilentlyContinue; if(-not(Test-Path -LiteralPath (Join-Path $env:ERRATA_NODE_HOME 'node.exe'))){ throw 'node.exe was not found after extraction' }"
if errorlevel 1 (
